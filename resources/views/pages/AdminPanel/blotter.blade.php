      ...
      <div class="modal-content">
         ...
         <div class="modal-body">
            <div class="print-header" style="display: none;">
               <h2 class="text-center">Barangay Blotter Report</h2>
               <p class="text-center">Date Printed: <span id="print-date"></span></p>
               <hr>
            </div>
            ...
            var table = $('.data-table').DataTable({
             ...
             $('body').on('click', '.viewBlotter', function(){
               ...
               $.get("{{ route('blotters.index') }}" +'/' + blotter_id +'/edit', function (data) {
                  ...
                  var timeIncident = formatTime(data[0].time_incident);
                  var timeReported = formatTime(data[0].time_reported);
                  ...
                  $('#viewtimeof_incident').html(timeIncident);
                  $('#viewdate_reported').html(data[0].date_reported);
                  $('#viewtime_reported').html(timeReported);
                  $('#viewschedule_date').html(data[0].schedule_date);
                  $('#viewincident_narrative').val(data[0].incident_narrative);

                  // Clear and populate persons involved table
                  $('.blotter-list-data').empty();
                  if(data[1] && data[1].length > 0) {
                      data[1].forEach(function(person) {
                          $('.blotter-list-data').append(`
                              <tr>
                                  <td>${person.resident_id}</td>
                                  <td>${person.person_involve}</td>
                                  <td>${person.involvement_type}</td>
                              </tr>
                          `);
                      });
                  } else {
                      $('.blotter-list-data').append('<tr><td colspan="3" class="text-center">No persons involved</td></tr>');
                  }
               });
             });
...
@media print {
    ...
    .modal-header .close,
    .modal-header .print-button,
    .modal-footer,
    #blotter_list-table th:first-child,
    #blotter_list-table td:first-child {
...
function exportToPDF() {
    ...
    const element = document.querySelector('#viewblottermodal .modal-body');
