...
$('#regionsaves').click(function (e) {
  ...
  $(document).ready(function() {
    $('.resident-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('residents.datatable') }}",
        columns: [
            {data: 'checkbox', name: 'checkbox', orderable: false, searchable: false},
            {data: 'action', name: 'action', orderable: false, searchable: false},
            {data: 'resident_id', name: 'resident_id'}, // Make sure this matches your API response
        ...
        columnDefs: [
            {
                targets: 2, // Resident ID column
                render: function(data, type, row) {
                    return data || 'RES-' + String(row.id).padStart(4, '0');
                }
            }
        ]
    });
