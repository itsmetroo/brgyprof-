        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn .= ' <a href="javascript:void(0)" data-toggle="tooltip" data-id="'.$row->resident_id.'" data-original-title="View" class="btn btn-primary btn-xs pr-4 pl-4 viewresident"><i class="fa fa-folder fa-lg"></i></a>';
                        return $btn;
                    })
                    ->addColumn('resident_id', function($row){
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'mobile'  => 'required',
             'height'  => 'required',
             'weight'  => 'required',
            'PAG_IBIG'    => 'nullable|string|max:14',
            'PHILHEALTH'  => 'nullable|string|max:14',
            'SSS' => 'nullable|string|max:12',
    ...
    public function datatable()
{
    return DataTables::of(Resident::query())
        ->addColumn('checkbox', function($resident) {
            return '<input type="checkbox" class="icheckbox_flat-green">';
        })
        ->addColumn('action', function($resident) {
            return '<button class="btn btn-sm btn-primary">View</button>
                    <button class="btn btn-sm btn-warning">Edit</button>';
        })
        ->addColumn('resident_id', function($resident) {
            return 'RES-' . str_pad($resident->id, 4, '0', STR_PAD_LEFT);
        })
        ->rawColumns(['checkbox', 'action'])
        ->make(true);
}
